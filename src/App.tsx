...
import type { Booking, Passenger, SearchValues, Seat, Ticket, User } from './types';

type Page = 'home' | 'results' | 'details' | 'seats' | 'passengers' | 'review' | 'confirmation' | 'bookings' | 'booking-detail' | 'auth';
...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="site-header"><button className="brand" onClick={goHome}>Ticket<span>Flow</span></button><nav><button onClick={goHome}>Search</button><button onClick={openBookings}>My bookings</button>{user ? <><button className="account-button" onClick={openBookings} title="Open your bookings"><i>{user.fullName.charAt(0).toUpperCase()}</i><span>{user.fullName.split(' ')[0]}</span></button><button className="nav-login" onClick={logout} disabled={isLoading}>Log out</button></> : <button className="nav-login" onClick={() => openAuthentication('home')}>Log in</button>}</nav></header>
